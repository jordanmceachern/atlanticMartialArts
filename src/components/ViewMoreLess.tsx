...
export const ViewMoreLess = ({ children, index, title }: ViewMoreLessProps) => {
  ...
  const viewMore = () => setIsOpen(true);
  const viewLess = () => setIsOpen(false);
  const flip = { transform: 'rotate(180deg)' };
  const contentRef = useRef<HTMLDivElement | null>(null);
  const scrollToTop = () => {
    contentRef?.current?.firstElementChild?.scrollIntoView();
    viewLess();
  };
  ...
  return (
    <div className='flex flex-col flex-grow w-full h-full justify-start items-center'>
      ...
      <input className={`view-more-less-${index} hidden`} checked={isOpen} aria-checked={isOpen} type='checkbox' name={`view-more-less-${index}`} id={`view-more-less-${index}`} />
      <div ref={contentRef} className={`view-content-${index} w-full flex flex-col items-start text-start`}>
        {children}
      </div>
      <label onClick={viewMore} className={`view-more-${index} text-center w-full border-white/20 border-t whitespace-nowrap cursor-pointer`} htmlFor={`view-more-less-${index}`} aria-label='toggle view more'>
