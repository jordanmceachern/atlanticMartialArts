...
const ScheduleTimes = ({ isBannerWidth }: ScheduleTimesProps) => (
  <div
    className={`flex flex-col h-full flex-grow items-center justify-center ${isBannerWidth ? '' : 'rounded-lg'}`}
    style={{
      background:
        'center / cover no-repeat url(https://res.cloudinary.com/dtweazqf2/image/upload/q_auto,f_auto/v1695320611/turny_kick_qmxovz.jpg)',
    }}
  >
    <ul
      aria-label='class schedule'
      className={`flex flex-col bg-black/50 w-full p-4 ${isBannerWidth ? 'md:w-4/5 lg:w-3/4 xl:w-1/2 mx-16 sm:mx-0' : 'h-full pt-auto'} rounded-lg items-start text-white`}
    >
...
const ScheduleContent = () => (
  <div className='h-72 sm:h-96 bg-black rounded-lg flex justify-center w-full'>
    <div className='flex h-full w-full sm:w-3/4 md:w-1/2'>
